...
var resizePizzas = function(size) {
  ...
  //and returns a new width of the pizza
  ...
  function changePizzaSizes(size) {
    ...
    //better performance of getElementsByClassName over querySelectorAll
    //http://ryanmorr.com/abstract-away-the-performance-faults-of-queryselectorall/
    var elePizza = document.getElementsByClassName("randomPizzaContainer");

    for (var i = 0, elePizzaLen = elePizza.length; i < elePizzaLen; i++) {
      //changes the width of each pizza with width returning from changeSliderLabel
      elePizza[i].style.width = newWidth + '%';
    }
  }
...
//function to convert of live node list to an array
//http://jsperf.com/nodelist-vs-array-iteration
...
function updatePositions() {
  ...
  //creates an array of 5 different elements
  for (var i = 0; i < 5; i++){
    phase.push(Math.sin(docScroll + i) * 300);
  }

  //adding transfor:translate3d() property to each pizza for reducing the paint and composite
  //layer task and lets each sliding pizza to be rendered on its own layer and thus prevent
  //whole document to be painted in every scroll.
  for (var i = 0, itemsLen = items.length; i < itemsLen; i++) {
    items[i].style.webkitTransform = "translate3d(" + phase[i % 5] + "px,0px,0px)";
    items[i].style.msTransform = "translate3d(" + phase[i % 5] + "px,0px,0px)";
    items[i].style.MozTransform = "translate3d(" + phase[i % 5] + "px,0px,0px)";
    items[i].style.OTransform = "translate3d(" + phase[i % 5] + "px,0px,0px)";
    items[i].style.transform = "translate3d(" + phase[i % 5] + "px,0px,0px)";
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  //dynamically creating total number of sliding pizza based on
  //width and height of the screen of the device
  var totalCol = screen.width / 256 + 1;
  var totalRow = screen.height / 256 + 1;

  //spacing between rows and colums
  var s = 256;
  for (var i = 0, totalPizzas = totalRow*totalCol; i < totalPizzas; i++) {
    ...
    /*The will-change CSS property hints the  browser about the
    kind of changes to be expected on an element and thus let the browser
    setup appropriate optimizations ahead of time before the element
    is actually changed. These kind of optimizations can increase the
    responsiveness of a page by doing potentially expensive work ahead
    of time before they are actually required*/
